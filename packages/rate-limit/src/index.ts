...
/**
 * Factory instead of a module-scope singleton: Cloudflare Workers deliver
 * credentials per-request via `env`/bindings, not `process.env` available
 * at module load time the way Next.js's Node runtime does — so each app
 * (web, api) constructs its own instance from whatever env shape it has.
 */
export function createRateLimit(credentials: RedisCredentials) {
  ...
  function getRatelimit(limit: number, window: Duration): Ratelimit {
    ...
    if (!instance) {
      ...
      instance = new Ratelimit({
        ...
        // Same Map reused for this config across requests on a warm serverless/Worker instance.
