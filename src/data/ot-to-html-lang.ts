/*
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

export interface OtToHtmlLangEntry {
  html: string;
  name: string;
  ot: string;
}

// Ported from wakamai-fondue-engine's ot-to-html-lang.js (Apache-2.0, Google LLC
// per header above) - a static registry mapping 4-char OpenType langSys tags
// to BCP-47/HTML lang codes, used by getLanguageSystems() to translate a
// font's declared GSUB/GPOS language-system support into readable entries.
export const OT_TO_HTML_LANG: OtToHtmlLangEntry[] = [
	{
		html: "abq",
		name: "Abaza",
		ot: "ABA",
	},
	{
		html: "ab",
		name: "Abkhazian",
		ot: "ABK",
	},
	{
		html: "acr",
		name: "Achi",
		ot: "ACR",
	},
	{
		html: "acy",
		name: "Cypriot Arabic",
		ot: "ACY",
	},
	{
		html: "af",
		name: "Afrikaans",
		ot: "AFK",
	},
	{
		html: "aa",
		name: "Afar",
		ot: "AFR",
	},
	{
		html: "ahg",
		name: "Agaw",
		ot: "AGW",
	},
	{
		html: "ak",
		name: "Akan",
		ot: "AKA",
	},
	{
		html: "akb",
		name: "Batak Angkola",
		ot: "AKB",
	},
	{
		html: "gsw",
		name: "Alsatian",
		ot: "ALS",
	},
	{
		html: "alt",
		name: "Southern Altai",
		ot: "ALT",
	},
	{
		html: "am",
		name: "Amharic",
		ot: "AMH",
	},
	{
		html: "und-fonnapa",
		name: "Undetermined; North American Phonetic Alphabet",
		ot: "APPH",
	},
	{
		html: "ar",
		name: "Arabic",
		ot: "ARA",
	},
	{
		html: "an",
		name: "Aragonese",
		ot: "ARG",
	},
	{
		html: "aiw",
		name: "Aari",
		ot: "ARI",
	},
	{
		html: "rki",
		name: "Rakhine",
		ot: "ARK",
	},
	{
		html: "as",
		name: "Assamese",
		ot: "ASM",
	},
	{
		html: "ath",
		name: "Athapascan",
		ot: "ATH",
	},
	{
		html: "av",
		name: "Avar",
		ot: "AVR",
	},
	{
		html: "ay",
		name: "Aymara",
		ot: "AYM",
	},
	{
		html: "azb",
		name: "Torki",
		ot: "AZB",
	},
	{
		html: "az",
		name: "Azerbaijani",
		ot: "AZE",
	},
	{
		html: "bfq",
		name: "Badaga",
		ot: "BAD",
	},
	{
		html: "bad",
		name: "Banda",
		ot: "BAD0",
	},
	{
		html: "bfy",
		name: "Baghelkhandi",
		ot: "BAG",
	},
	{
		html: "krc",
		name: "Balkar",
		ot: "BAL",
	},
	{
		html: "bci",
		name: "Baul\u00e9",
		ot: "BAU",
	},
	{
		html: "bbc",
		name: "Batak Toba",
		ot: "BBC",
	},
	{
		html: "ber",
		name: "Berber",
		ot: "BBR",
	},
	{
		html: "bcq",
		name: "Bench",
		ot: "BCH",
	},
	{
		html: "be",
		name: "Belarusian",
		ot: "BEL",
	},
	{
		html: "bn",
		name: "Bangla",
		ot: "BEN",
	},
	{
		html: "bgq",
		name: "Bagri",
		ot: "BGQ",
	},
	{
		html: "bg",
		name: "Bulgarian",
		ot: "BGR",
	},
	{
		html: "bhb",
		name: "Bhili",
		ot: "BHI",
	},
	{
		html: "bik",
		name: "Bikol",
		ot: "BIK",
	},
	{
		html: "byn",
		name: "Bilen",
		ot: "BIL",
	},
	{
		html: "bi",
		name: "Bislama",
		ot: "BIS",
	},
	{
		html: "bla",
		name: "Blackfoot",
		ot: "BKF",
	},
	{
		html: "bal",
		name: "Baluchi",
		ot: "BLI",
	},
	{
		html: "blk",
		name: "Pa\u2019o Karen",
		ot: "BLK",
	},
	{
		html: "bjt",
		name: "Balante",
		ot: "BLN",
	},
	{
		html: "bft",
		name: "Balti",
		ot: "BLT",
	},
	{
		html: "bm",
		name: "Bambara (Bamanankan)",
		ot: "BMB",
	},
	{
		html: "bai",
		name: "Bamileke",
		ot: "BML",
	},
	{
		html: "bs",
		name: "Bosnian",
		ot: "BOS",
	},
	{
		html: "br",
		name: "Breton",
		ot: "BRE",
	},
	{
		html: "bra",
		name: "Braj Bhasha",
		ot: "BRI",
	},
	{
		html: "my",
		name: "Burmese",
		ot: "BRM",
	},
	{
		html: "ba",
		name: "Bashkir",
		ot: "BSH",
	},
	{
		html: "btd",
		name: "Batak Dairi (Pakpak)",
		ot: "BTD",
	},
	{
		html: "beb",
		name: "Beti",
		ot: "BTI",
	},
	{
		html: "btk",
		name: "Batak",
		ot: "BTK",
	},
	{
		html: "btm",
		name: "Batak Mandailing",
		ot: "BTM",
	},
	{
		html: "bts",
		name: "Batak Simalungun",
		ot: "BTS",
	},
	{
		html: "btx",
		name: "Batak Karo",
		ot: "BTX",
	},
	{
		html: "btz",
		name: "Batak Alas-Kluet",
		ot: "BTZ",
	},
	{
		html: "byv",
		name: "Medumba",
		ot: "BYV",
	},
	{
		html: "cak",
		name: "Kaqchikel",
		ot: "CAK",
	},
	{
		html: "ca",
		name: "Catalan",
		ot: "CAT",
	},
	{
		html: "cbk",
		name: "Zamboanga Chavacano",
		ot: "CBK",
	},
	{
		html: "cco",
		name: "Chinantec",
		ot: "CCHN",
	},
	{
		html: "ch",
		name: "Chamorro",
		ot: "CHA",
	},
	{
		html: "ce",
		name: "Chechen",
		ot: "CHE",
	},
	{
		html: "sgw",
		name: "Chaha Gurage",
		ot: "CHG",
	},
	{
		html: "hne",
		name: "Chattisgarhi",
		ot: "CHH",
	},
	{
		html: "ny",
		name: "Chichewa (Chewa, Nyanja)",
		ot: "CHI",
	},
	{
		html: "ckt",
		name: "Chukchi",
		ot: "CHK",
	},
	{
		html: "chk",
		name: "Chuukese",
		ot: "CHK0",
	},
	{
		html: "chp",
		name: "Chipewyan",
		ot: "CHP",
	},
	{
		html: "cv",
		name: "Chuvash",
		ot: "CHU",
	},
	{
		html: "swb",
		name: "Comorian",
		ot: "CMR",
	},
	{
		html: "kw",
		name: "Cornish",
		ot: "COR",
	},
	{
		html: "co",
		name: "Corsican",
		ot: "COS",
	},
	{
		html: "crp",
		name: "Creoles and pidgins",
		ot: "CPP",
	},
	{
		html: "cr",
		name: "Cree",
		ot: "CRE",
	},
	{
		html: "crx",
		name: "Carrier",
		ot: "CRR",
	},
	{
		html: "crh",
		name: "Crimean Tatar",
		ot: "CRT",
	},
	{
		html: "cu",
		name: "Church Slavonic",
		ot: "CSL",
	},
	{
		html: "cs",
		name: "Czech",
		ot: "CSY",
	},
	{
		html: "da",
		name: "Danish",
		ot: "DAN",
	},
	{
		html: "cwd",
		name: "Woods Cree",
		ot: "DCR",
	},
	{
		html: "de",
		name: "German",
		ot: "DEU",
	},
	{
		html: "dgo",
		name: "Dogri (individual language)",
		ot: "DGO",
	},
	{
		html: "doi",
		name: "Dogri",
		ot: "DGR",
	},
	{
		html: "dv",
		name: "Divehi (Dhivehi, Maldivian)",
		ot: "DHV",
	},
	{
		html: "diq",
		name: "Dimli",
		ot: "DIQ",
	},
	{
		html: "dv",
		name: "Divehi (Dhivehi, Maldivian)",
		ot: "DIV",
	},
	{
		html: "dje",
		name: "Zarma",
		ot: "DJR",
	},
	{
		html: "djr",
		name: "Djambarrpuyngu",
		ot: "DJR0",
	},
	{
		html: "ada",
		name: "Dangme",
		ot: "DNG",
	},
	{
		html: "din",
		name: "Dinka",
		ot: "DNK",
	},
	{
		html: "prs",
		name: "Dari",
		ot: "DRI",
	},
	{
		html: "dwu",
		name: "Dhuwal",
		ot: "DUJ",
	},
	{
		html: "dng",
		name: "Dungan",
		ot: "DUN",
	},
	{
		html: "dz",
		name: "Dzongkha",
		ot: "DZN",
	},
	{
		html: "igb",
		name: "Ebira",
		ot: "EBI",
	},
	{
		html: "crj",
		name: "Eastern Cree",
		ot: "ECR",
	},
	{
		html: "bin",
		name: "Edo",
		ot: "EDO",
	},
	{
		html: "el",
		name: "Greek",
		ot: "ELL",
	},
	{
		html: "emk",
		name: "Eastern Maninkakan",
		ot: "EMK",
	},
	{
		html: "en",
		name: "English",
		ot: "ENG",
	},
	{
		html: "myv",
		name: "Erzya",
		ot: "ERZ",
	},
	{
		html: "es",
		name: "Spanish",
		ot: "ESP",
	},
	{
		html: "et",
		name: "Estonian",
		ot: "ETI",
	},
	{
		html: "eu",
		name: "Basque",
		ot: "EUQ",
	},
	{
		html: "evn",
		name: "Evenki",
		ot: "EVK",
	},
	{
		html: "eve",
		name: "Even",
		ot: "EVN",
	},
	{
		html: "ee",
		name: "Ewe",
		ot: "EWE",
	},
	{
		html: "acf",
		name: "French Antillean",
		ot: "FAN",
	},
	{
		html: "fan",
		name: "Fang (Equatorial Guinea)",
		ot: "FAN0",
	},
	{
		html: "fa",
		name: "Persian",
		ot: "FAR",
	},
	{
		html: "fat",
		name: "Fanti",
		ot: "FAT",
	},
	{
		html: "fi",
		name: "Finnish",
		ot: "FIN",
	},
	{
		html: "fj",
		name: "Fijian",
		ot: "FJI",
	},
	{
		html: "vls",
		name: "Dutch (Flemish)",
		ot: "FLE",
	},
	{
		html: "fmp",
		name: "Fe\u2019fe\u2019",
		ot: "FMP",
	},
	{
		html: "enf",
		name: "Forest Enets",
		ot: "FNE",
	},
	{
		html: "fo",
		name: "Faroese",
		ot: "FOS",
	},
	{
		html: "fr",
		name: "French",
		ot: "FRA",
	},
	{
		html: "fy",
		name: "Frisian",
		ot: "FRI",
	},
	{
		html: "fur",
		name: "Friulian",
		ot: "FRL",
	},
	{
		html: "fuf",
		name: "Futa",
		ot: "FTA",
	},
	{
		html: "ff",
		name: "Fulah",
		ot: "FUL",
	},
	{
		html: "fuv",
		name: "Nigerian Fulfulde",
		ot: "FUV",
	},
	{
		html: "gaa",
		name: "Ga",
		ot: "GAD",
	},
	{
		html: "gd",
		name: "Scottish Gaelic",
		ot: "GAE",
	},
	{
		html: "gl",
		name: "Galician",
		ot: "GAL",
	},
	{
		html: "gbm",
		name: "Garhwali",
		ot: "GAW",
	},
	{
		html: "niv",
		name: "Gilyak",
		ot: "GIL",
	},
	{
		html: "gil",
		name: "Kiribati (Gilbertese)",
		ot: "GIL0",
	},
	{
		html: "gkp",
		name: "Kpelle (Guinea)",
		ot: "GKP",
	},
	{
		html: "guk",
		name: "Gumuz",
		ot: "GMZ",
	},
	{
		html: "gon",
		name: "Gondi",
		ot: "GON",
	},
	{
		html: "kl",
		name: "Greenlandic",
		ot: "GRN",
	},
	{
		html: "grt",
		name: "Garo",
		ot: "GRO",
	},
	{
		html: "gn",
		name: "Guarani",
		ot: "GUA",
	},
	{
		html: "gu",
		name: "Gujarati",
		ot: "GUJ",
	},
	{
		html: "ht",
		name: "Haitian (Haitian Creole)",
		ot: "HAI",
	},
	{
		html: "hai",
		name: "Haida",
		ot: "HAI0",
	},
	{
		html: "cfm",
		name: "Halam (Falam Chin)",
		ot: "HAL",
	},
	{
		html: "hoj",
		name: "Harauti",
		ot: "HAR",
	},
	{
		html: "ha",
		name: "Hausa",
		ot: "HAU",
	},
	{
		html: "amf",
		name: "Hammer-Banna",
		ot: "HBN",
	},
	{
		html: "hz",
		name: "Herero",
		ot: "HER",
	},
	{
		html: "hi",
		name: "Hindi",
		ot: "HIN",
	},
	{
		html: "mrj",
		name: "Western Mari",
		ot: "HMA",
	},
	{
		html: "hmd",
		name: "A-Hmao",
		ot: "HMD",
	},
	{
		html: "hmn",
		name: "Hmong",
		ot: "HMN",
	},
	{
		html: "ho",
		name: "Hiri Motu",
		ot: "HMO",
	},
	{
		html: "hmz",
		name: "Hmong Shuat",
		ot: "HMZ",
	},
	{
		html: "hnd",
		name: "Southern Hindko",
		ot: "HND",
	},
	{
		html: "hoc",
		name: "Ho",
		ot: "HO",
	},
	{
		html: "har",
		name: "Harari",
		ot: "HRI",
	},
	{
		html: "hr",
		name: "Croatian",
		ot: "HRV",
	},
	{
		html: "hu",
		name: "Hungarian",
		ot: "HUN",
	},
	{
		html: "hyw",
		name: "Western Armenian",
		ot: "HYE",
	},
	{
		html: "hy",
		name: "Armenian East",
		ot: "HYE0",
	},
	{
		html: "iba",
		name: "Iban",
		ot: "IBA",
	},
	{
		html: "ig",
		name: "Igbo",
		ot: "IBO",
	},
	{
		html: "io",
		name: "Ido",
		ot: "IDO",
	},
	{
		html: "ijo",
		name: "Ijo",
		ot: "IJO",
	},
	{
		html: "ie",
		name: "Interlingue",
		ot: "ILE",
	},
	{
		html: "ia",
		name: "Interlingua (International Auxiliary Language Association)",
		ot: "INA",
	},
	{
		html: "id",
		name: "Indonesian",
		ot: "IND",
	},
	{
		html: "inh",
		name: "Ingush",
		ot: "ING",
	},
	{
		html: "iu",
		name: "Inuktitut",
		ot: "INU",
	},
	{
		html: "iu",
		name: "Nunavik Inuktitut",
		ot: "INUK",
	},
	{
		html: "ik",
		name: "Inupiat",
		ot: "IPK",
	},
	{
		html: "und-fonipa",
		name: "Undetermined; International Phonetic Alphabet",
		ot: "IPPH",
	},
	{
		html: "ga",
		name: "Irish",
		ot: "IRI",
	},
	{
		html: "ghc",
		name: "Hiberno-Scottish Gaelic",
		ot: "IRT",
	},
	{
		html: "is",
		name: "Icelandic",
		ot: "ISL",
	},
	{
		html: "smn",
		name: "Inari Sami",
		ot: "ISM",
	},
	{
		html: "it",
		name: "Italian",
		ot: "ITA",
	},
	{
		html: "he",
		name: "Hebrew",
		ot: "IWR",
	},
	{
		html: "jam",
		name: "Jamaican Creole",
		ot: "JAM",
	},
	{
		html: "ja",
		name: "Japanese",
		ot: "JAN",
	},
	{
		html: "jv",
		name: "Javanese",
		ot: "JAV",
	},
	{
		html: "yi",
		name: "Yiddish",
		ot: "JII",
	},
	{
		html: "lad",
		name: "Ladino",
		ot: "JUD",
	},
	{
		html: "dyu",
		name: "Jula",
		ot: "JUL",
	},
	{
		html: "kbd",
		name: "Kabardian",
		ot: "KAB",
	},
	{
		html: "kab",
		name: "Kabyle",
		ot: "KAB0",
	},
	{
		html: "kfr",
		name: "Kachchi",
		ot: "KAC",
	},
	{
		html: "kln",
		name: "Kalenjin",
		ot: "KAL",
	},
	{
		html: "kn",
		name: "Kannada",
		ot: "KAN",
	},
	{
		html: "krc",
		name: "Karachay",
		ot: "KAR",
	},
	{
		html: "ka",
		name: "Georgian",
		ot: "KAT",
	},
	{
		html: "kk",
		name: "Kazakh",
		ot: "KAZ",
	},
	{
		html: "kea",
		name: "Kabuverdianu (Crioulo)",
		ot: "KEA",
	},
	{
		html: "ktb",
		name: "Kebena",
		ot: "KEB",
	},
	{
		html: "kek",
		name: "Kekchi",
		ot: "KEK",
	},
	{
		html: "und-Geok",
		name: "Undetermined; Khutsuri (Asomtavruli and Nuskhuri)",
		ot: "KGE",
	},
	{
		html: "kjh",
		name: "Khakass",
		ot: "KHA",
	},
	{
		html: "kca",
		name: "Khanty-Kazim",
		ot: "KHK",
	},
	{
		html: "km",
		name: "Khmer",
		ot: "KHM",
	},
	{
		html: "kht",
		name: "Khamti Shan (Microsoft fonts)",
		ot: "KHN",
	},
	{
		html: "kca",
		name: "Khanty-Shurishkar",
		ot: "KHS",
	},
	{
		html: "kht",
		name: "Khamti Shan",
		ot: "KHT",
	},
	{
		html: "kca",
		name: "Khanty-Vakhi",
		ot: "KHV",
	},
	{
		html: "ki",
		name: "Kikuyu (Gikuyu)",
		ot: "KIK",
	},
	{
		html: "ky",
		name: "Kirghiz (Kyrgyz)",
		ot: "KIR",
	},
	{
		html: "kqs",
		name: "Kisii",
		ot: "KIS",
	},
	{
		html: "kiu",
		name: "Kirmanjki",
		ot: "KIU",
	},
	{
		html: "kjp",
		name: "Eastern Pwo Karen",
		ot: "KJP",
	},
	{
		html: "kex",
		name: "Kokni",
		ot: "KKN",
	},
	{
		html: "xal",
		name: "Kalmyk",
		ot: "KLM",
	},
	{
		html: "kam",
		name: "Kamba (Kenya)",
		ot: "KMB",
	},
	{
		html: "kfy",
		name: "Kumaoni",
		ot: "KMN",
	},
	{
		html: "kmw",
		name: "Komo (Democratic Republic of Congo)",
		ot: "KMO",
	},
	{
		html: "kxc",
		name: "Komso",
		ot: "KMS",
	},
	{
		html: "kr",
		name: "Kanuri",
		ot: "KNR",
	},
	{
		html: "kfa",
		name: "Kodagu",
		ot: "KOD",
	},
	{
		html: "okm",
		name: "Middle Korean (10th-16th cent.)",
		ot: "KOH",
	},
	{
		html: "kok",
		name: "Konkani",
		ot: "KOK",
	},
	{
		html: "kv",
		name: "Komi",
		ot: "KOM",
	},
	{
		html: "ktu",
		name: "Kikongo",
		ot: "KON",
	},
	{
		html: "kg",
		name: "Kongo",
		ot: "KON0",
	},
	{
		html: "koi",
		name: "Komi-Permyak",
		ot: "KOP",
	},
	{
		html: "ko",
		name: "Korean",
		ot: "KOR",
	},
	{
		html: "kpv",
		name: "Komi-Zyrian",
		ot: "KOZ",
	},
	{
		html: "kpe",
		name: "Kpelle",
		ot: "KPL",
	},
	{
		html: "kri",
		name: "Krio",
		ot: "KRI",
	},
	{
		html: "kaa",
		name: "Karakalpak",
		ot: "KRK",
	},
	{
		html: "kdr",
		name: "Karaim",
		ot: "KRM",
	},
	{
		html: "kar",
		name: "Karen",
		ot: "KRN",
	},
	{
		html: "kqy",
		name: "Koorete",
		ot: "KRT",
	},
	{
		html: "ks",
		name: "Kashmiri",
		ot: "KSH",
	},
	{
		html: "ksh",
		name: "Ripuarian",
		ot: "KSH0",
	},
	{
		html: "kha",
		name: "Khasi",
		ot: "KSI",
	},
	{
		html: "sjd",
		name: "Kildin Sami",
		ot: "KSM",
	},
	{
		html: "ksw",
		name: "S\u2019gaw Karen",
		ot: "KSW",
	},
	{
		html: "kj",
		name: "Kuanyama",
		ot: "KUA",
	},
	{
		html: "uki",
		name: "Kui (India)",
		ot: "KUI",
	},
	{
		html: "kfx",
		name: "Kulvi",
		ot: "KUL",
	},
	{
		html: "ku",
		name: "Kurdish",
		ot: "KUR",
	},
	{
		html: "kru",
		name: "Kurukh",
		ot: "KUU",
	},
	{
		html: "kdt",
		name: "Kuy",
		ot: "KUY",
	},
	{
		html: "kvq",
		name: "Geba Karen",
		ot: "KVQ",
	},
	{
		html: "kpy",
		name: "Koryak",
		ot: "KYK",
	},
	{
		html: "kyu",
		name: "Western Kayah",
		ot: "KYU",
	},
	{
		html: "lld",
		name: "Ladin",
		ot: "LAD",
	},
	{
		html: "bfu",
		name: "Lahuli",
		ot: "LAH",
	},
	{
		html: "lbe",
		name: "Lak",
		ot: "LAK",
	},
	{
		html: "lmn",
		name: "Lambani",
		ot: "LAM",
	},
	{
		html: "lo",
		name: "Lao",
		ot: "LAO",
	},
	{
		html: "la",
		name: "Latin",
		ot: "LAT",
	},
	{
		html: "lzz",
		name: "Laz",
		ot: "LAZ",
	},
	{
		html: "crm",
		name: "L-Cree",
		ot: "LCR",
	},
	{
		html: "lbj",
		name: "Ladakhi",
		ot: "LDK",
	},
	{
		html: "li",
		name: "Limburgish",
		ot: "LIM",
	},
	{
		html: "ln",
		name: "Lingala",
		ot: "LIN",
	},
	{
		html: "mhr",
		name: "Eastern Mari",
		ot: "LMA",
	},
	{
		html: "lif",
		name: "Limbu",
		ot: "LMB",
	},
	{
		html: "ngl",
		name: "Lomwe",
		ot: "LMW",
	},
	{
		html: "bqi",
		name: "Luri",
		ot: "LRC",
	},
	{
		html: "dsb",
		name: "Lower Sorbian",
		ot: "LSB",
	},
	{
		html: "smj",
		name: "Lule Sami",
		ot: "LSM",
	},
	{
		html: "lt",
		name: "Lithuanian",
		ot: "LTH",
	},
	{
		html: "lb",
		name: "Luxembourgish",
		ot: "LTZ",
	},
	{
		html: "lu",
		name: "Luba-Katanga",
		ot: "LUB",
	},
	{
		html: "lg",
		name: "Ganda",
		ot: "LUG",
	},
	{
		html: "luy",
		name: "Luyia",
		ot: "LUH",
	},
	{
		html: "lv",
		name: "Latvian",
		ot: "LVI",
	},
	{
		html: "mh",
		name: "Marshallese",
		ot: "MAH",
	},
	{
		html: "mpe",
		name: "Majang",
		ot: "MAJ",
	},
	{
		html: "vmw",
		name: "Makhuwa",
		ot: "MAK",
	},
	{
		html: "ml",
		name: "Malayalam Traditional",
		ot: "MAL",
	},
	{
		html: "mam",
		name: "Mam",
		ot: "MAM",
	},
	{
		html: "mns",
		name: "Mansi",
		ot: "MAN",
	},
	{
		html: "arn",
		name: "Mapudungun",
		ot: "MAP",
	},
	{
		html: "mr",
		name: "Marathi",
		ot: "MAR",
	},
	{
		html: "mwr",
		name: "Marwari",
		ot: "MAW",
	},
	{
		html: "kmb",
		name: "Mbundu",
		ot: "MBN",
	},
	{
		html: "mnc",
		name: "Manchu",
		ot: "MCH",
	},
	{
		html: "crm",
		name: "Moose Cree",
		ot: "MCR",
	},
	{
		html: "men",
		name: "Mende (Sierra Leone)",
		ot: "MDE",
	},
	{
		html: "mym",
		name: "Me\u2019en",
		ot: "MEN",
	},
	{
		html: "mfa",
		name: "Pattani Malay",
		ot: "MFA",
	},
	{
		html: "mfe",
		name: "Morisyen",
		ot: "MFE",
	},
	{
		html: "min",
		name: "Minangkabau",
		ot: "MIN",
	},
	{
		html: "lus",
		name: "Mizo",
		ot: "MIZ",
	},
	{
		html: "mk",
		name: "Macedonian",
		ot: "MKD",
	},
	{
		html: "mak",
		name: "Makasar",
		ot: "MKR",
	},
	{
		html: "mdy",
		name: "Male (Ethiopia)",
		ot: "MLE",
	},
	{
		html: "mg",
		name: "Malagasy",
		ot: "MLG",
	},
	{
		html: "mlq",
		name: "Malinke",
		ot: "MLN",
	},
	{
		html: "ml",
		name: "Malayalam Reformed",
		ot: "MLR",
	},
	{
		html: "ms",
		name: "Malay",
		ot: "MLY",
	},
	{
		html: "mnk",
		name: "Mandinka",
		ot: "MND",
	},
	{
		html: "mn",
		name: "Mongolian",
		ot: "MNG",
	},
	{
		html: "man",
		name: "Mandingo",
		ot: "MNK",
	},
	{
		html: "gv",
		name: "Manx",
		ot: "MNX",
	},
	{
		html: "mdf",
		name: "Moksha",
		ot: "MOK",
	},
	{
		html: "ro-MD",
		name: "Romanian; Moldova",
		ot: "MOL",
	},
	{
		html: "mnw",
		name: "Mon",
		ot: "MON",
	},
	{
		html: "mnw-TH",
		name: "Mon; Thailand",
		ot: "MONT",
	},
	{
		html: "ary",
		name: "Moroccan",
		ot: "MOR",
	},
	{
		html: "mi",
		name: "Maori",
		ot: "MRI",
	},
	{
		html: "mai",
		name: "Maithili",
		ot: "MTH",
	},
	{
		html: "mt",
		name: "Maltese",
		ot: "MTS",
	},
	{
		html: "unr",
		name: "Mundari",
		ot: "MUN",
	},
	{
		html: "mww",
		name: "Hmong Daw",
		ot: "MWW",
	},
	{
		html: "myn",
		name: "Mayan",
		ot: "MYN",
	},
	{
		html: "nag",
		name: "Naga-Assamese",
		ot: "NAG",
	},
	{
		html: "nah",
		name: "Nahuatl",
		ot: "NAH",
	},
	{
		html: "gld",
		name: "Nanai",
		ot: "NAN",
	},
	{
		html: "nsk",
		name: "Naskapi",
		ot: "NAS",
	},
	{
		html: "na",
		name: "Nauruan",
		ot: "NAU",
	},
	{
		html: "nv",
		name: "Navajo",
		ot: "NAV",
	},
	{
		html: "csw",
		name: "N-Cree",
		ot: "NCR",
	},
	{
		html: "nd",
		name: "Ndebele",
		ot: "NDB",
	},
	{
		html: "ng",
		name: "Ndonga",
		ot: "NDG",
	},
	{
		html: "ne",
		name: "Nepali",
		ot: "NEP",
	},
	{
		html: "csw",
		name: "Norway House Cree",
		ot: "NHC",
	},
	{
		html: "njz",
		name: "Nyishi",
		ot: "NIS",
	},
	{
		html: "nyn",
		name: "Nyankole",
		ot: "NKL",
	},
	{
		html: "nqo",
		name: "N\u2019Ko",
		ot: "NKO",
	},
	{
		html: "nl",
		name: "Dutch",
		ot: "NLD",
	},
	{
		html: "no",
		name: "Norwegian",
		ot: "NOR",
	},
	{
		html: "se",
		name: "Northern Sami",
		ot: "NSM",
	},
	{
		html: "nod",
		name: "Northern Tai",
		ot: "NTA",
	},
	{
		html: "eo",
		name: "Esperanto",
		ot: "NTO",
	},
	{
		html: "nn",
		name: "Norwegian Nynorsk (Nynorsk, Norwegian)",
		ot: "NYN",
	},
	{
		html: "oc",
		name: "Occitan (post 1500)",
		ot: "OCI",
	},
	{
		html: "ojs",
		name: "Oji-Cree",
		ot: "OCR",
	},
	{
		html: "oj",
		name: "Ojibway",
		ot: "OJB",
	},
	{
		html: "or",
		name: "Odia",
		ot: "ORI",
	},
	{
		html: "om",
		name: "Oromo",
		ot: "ORO",
	},
	{
		html: "os",
		name: "Ossetian",
		ot: "OSS",
	},
	{
		html: "sam",
		name: "Palestinian Aramaic",
		ot: "PAA",
	},
	{
		html: "pi",
		name: "Pali",
		ot: "PAL",
	},
	{
		html: "pa",
		name: "Punjabi",
		ot: "PAN",
	},
	{
		html: "plp",
		name: "Palpa",
		ot: "PAP",
	},
	{
		html: "pap",
		name: "Papiamentu",
		ot: "PAP0",
	},
	{
		html: "ps",
		name: "Pashto",
		ot: "PAS",
	},
	{
		html: "el-polyton",
		name: "Modern Greek (1453-); Polytonic Greek",
		ot: "PGR",
	},
	{
		html: "pih",
		name: "Norfolk",
		ot: "PIH",
	},
	{
		html: "fil",
		name: "Filipino",
		ot: "PIL",
	},
	{
		html: "pce",
		name: "Palaung",
		ot: "PLG",
	},
	{
		html: "plg",
		name: "Pilaga\u0301",
		ot: "PLG0",
	},
	{
		html: "pl",
		name: "Polish",
		ot: "PLK",
	},
	{
		html: "poh",
		name: "Pocomchi",
		ot: "POH",
	},
	{
		html: "pro",
		name: "Old Proven\u00e7al (to 1500)",
		ot: "PRO",
	},
	{
		html: "pt",
		name: "Portuguese",
		ot: "PTG",
	},
	{
		html: "pwo",
		name: "Western Pwo Karen",
		ot: "PWO",
	},
	{
		html: "bgr",
		name: "Chin",
		ot: "QIN",
	},
	{
		html: "quc",
		name: "K\u2019iche\u2019",
		ot: "QUC",
	},
	{
		html: "quh",
		name: "South Bolivian Quechua",
		ot: "QUH",
	},
	{
		html: "qu",
		name: "Quechua",
		ot: "QUZ",
	},
	{
		html: "qvi",
		name: "Imbabura Highland Quichua",
		ot: "QVI",
	},
	{
		html: "qwh",
		name: "Huaylas Ancash Quechua",
		ot: "QWH",
	},
	{
		html: "raj",
		name: "Rajasthani",
		ot: "RAJ",
	},
	{
		html: "bxr",
		name: "Russian Buriat",
		ot: "RBU",
	},
	{
		html: "atj",
		name: "R-Cree",
		ot: "RCR",
	},
	{
		html: "rif",
		name: "Tarifit",
		ot: "RIF",
	},
	{
		html: "rm",
		name: "Romansh",
		ot: "RMS",
	},
	{
		html: "rmy",
		name: "Vlax Romani",
		ot: "RMY",
	},
	{
		html: "ro",
		name: "Romanian",
		ot: "ROM",
	},
	{
		html: "rom",
		name: "Romany",
		ot: "ROY",
	},
	{
		html: "rue",
		name: "Rusyn",
		ot: "RSY",
	},
	{
		html: "rw",
		name: "Kinyarwanda",
		ot: "RUA",
	},
	{
		html: "rn",
		name: "Rundi",
		ot: "RUN",
	},
	{
		html: "ru",
		name: "Russian",
		ot: "RUS",
	},
	{
		html: "sck",
		name: "Sadri",
		ot: "SAD",
	},
	{
		html: "sa",
		name: "Sanskrit",
		ot: "SAN",
	},
	{
		html: "chp",
		name: "Sayisi",
		ot: "SAY",
	},
	{
		html: "scs",
		name: "North Slavey",
		ot: "SCS",
	},
	{
		html: "xan",
		name: "Sekota",
		ot: "SEK",
	},
	{
		html: "sfm",
		name: "Small Flowery Miao",
		ot: "SFM",
	},
	{
		html: "sga",
		name: "Old Irish (to 900)",
		ot: "SGA",
	},
	{
		html: "sg",
		name: "Sango",
		ot: "SGO",
	},
	{
		html: "shi",
		name: "Tachelhit",
		ot: "SHI",
	},
	{
		html: "sjo",
		name: "Sibe",
		ot: "SIB",
	},
	{
		html: "stv",
		name: "Silte Gurage",
		ot: "SIG",
	},
	{
		html: "sms",
		name: "Skolt Sami",
		ot: "SKS",
	},
	{
		html: "sk",
		name: "Slovak",
		ot: "SKY",
	},
	{
		html: "den",
		name: "Slavey",
		ot: "SLA",
	},
	{
		html: "sl",
		name: "Slovenian",
		ot: "SLV",
	},
	{
		html: "so",
		name: "Somali",
		ot: "SML",
	},
	{
		html: "sm",
		name: "Samoan",
		ot: "SMO",
	},
	{
		html: "seh",
		name: "Sena",
		ot: "SNA",
	},
	{
		html: "sn",
		name: "Shona",
		ot: "SNA0",
	},
	{
		html: "sd",
		name: "Sindhi",
		ot: "SND",
	},
	{
		html: "si",
		name: "Sinhala (Sinhalese)",
		ot: "SNH",
	},
	{
		html: "gru",
		name: "Sodo Gurage",
		ot: "SOG",
	},
	{
		html: "st",
		name: "Southern Sotho",
		ot: "SOT",
	},
	{
		html: "sq",
		name: "Albanian",
		ot: "SQI",
	},
	{
		html: "sr",
		name: "Serbian",
		ot: "SRB",
	},
	{
		html: "sc",
		name: "Sardinian",
		ot: "SRD",
	},
	{
		html: "skr",
		name: "Saraiki",
		ot: "SRK",
	},
	{
		html: "xsl",
		name: "South Slavey",
		ot: "SSL",
	},
	{
		html: "sma",
		name: "Southern Sami",
		ot: "SSM",
	},
	{
		html: "su",
		name: "Sundanese",
		ot: "SUN",
	},
	{
		html: "suq",
		name: "Suri",
		ot: "SUR",
	},
	{
		html: "sv",
		name: "Swedish",
		ot: "SVE",
	},
	{
		html: "aii",
		name: "Swadaya Aramaic",
		ot: "SWA",
	},
	{
		html: "sw",
		name: "Swahili",
		ot: "SWK",
	},
	{
		html: "ss",
		name: "Swati",
		ot: "SWZ",
	},
	{
		html: "xnj",
		name: "Ngoni (Tanzania)",
		ot: "SXT",
	},
	{
		html: "syr",
		name: "Syriac",
		ot: "SYR",
	},
	{
		html: "und-Syre",
		name: "Undetermined; Syriac (Estrangelo variant)",
		ot: "SYRE",
	},
	{
		html: "und-Syrj",
		name: "Undetermined; Syriac (Western variant)",
		ot: "SYRJ",
	},
	{
		html: "und-Syrn",
		name: "Undetermined; Syriac (Eastern variant)",
		ot: "SYRN",
	},
	{
		html: "tg",
		name: "Tajiki",
		ot: "TAJ",
	},
	{
		html: "ta",
		name: "Tamil",
		ot: "TAM",
	},
	{
		html: "taq",
		name: "Tamasheq",
		ot: "TAQ",
	},
	{
		html: "tt",
		name: "Tatar",
		ot: "TAT",
	},
	{
		html: "cwd",
		name: "TH-Cree",
		ot: "TCR",
	},
	{
		html: "te",
		name: "Telugu",
		ot: "TEL",
	},
	{
		html: "tl",
		name: "Tagalog",
		ot: "TGL",
	},
	{
		html: "to",
		name: "Tongan",
		ot: "TGN",
	},
	{
		html: "tig",
		name: "Tigre",
		ot: "TGR",
	},
	{
		html: "ti",
		name: "Tigrinya",
		ot: "TGY",
	},
	{
		html: "th",
		name: "Thai",
		ot: "THA",
	},
	{
		html: "ty",
		name: "Tahitian",
		ot: "THT",
	},
	{
		html: "thv",
		name: "Tahaggart Tamahaq",
		ot: "THV",
	},
	{
		html: "thz",
		name: "Tayart Tamajeq",
		ot: "THZ",
	},
	{
		html: "bo",
		name: "Tibetan",
		ot: "TIB",
	},
	{
		html: "tk",
		name: "Turkmen",
		ot: "TKM",
	},
	{
		html: "tmh",
		name: "Tamashek",
		ot: "TMH",
	},
	{
		html: "tem",
		name: "Temne",
		ot: "TMN",
	},
	{
		html: "tn",
		name: "Tswana",
		ot: "TNA",
	},
	{
		html: "enh",
		name: "Tundra Enets",
		ot: "TNE",
	},
	{
		html: "toi",
		name: "Tonga (Zambia)",
		ot: "TNG",
	},
	{
		html: "xwo",
		name: "Written Oirat",
		ot: "TOD",
	},
	{
		html: "tod",
		name: "Toma",
		ot: "TOD0",
	},
	{
		html: "tpi",
		name: "Tok Pisin",
		ot: "TPI",
	},
	{
		html: "tr",
		name: "Turkish",
		ot: "TRK",
	},
	{
		html: "ts",
		name: "Tsonga",
		ot: "TSG",
	},
	{
		html: "ttq",
		name: "Tawallammat Tamajaq",
		ot: "TTQ",
	},
	{
		html: "tru",
		name: "Turoyo Aramaic",
		ot: "TUA",
	},
	{
		html: "tcy",
		name: "Tulu",
		ot: "TUL",
	},
	{
		html: "tyv",
		name: "Tuvin",
		ot: "TUV",
	},
	{
		html: "tw",
		name: "Twi",
		ot: "TWI",
	},
	{
		html: "tzm",
		name: "Tamazight",
		ot: "TZM",
	},
	{
		html: "tzo",
		name: "Tzotzil",
		ot: "TZO",
	},
	{
		html: "uk",
		name: "Ukrainian",
		ot: "UKR",
	},
	{
		html: "ur",
		name: "Urdu",
		ot: "URD",
	},
	{
		html: "hsb",
		name: "Upper Sorbian",
		ot: "USB",
	},
	{
		html: "ug",
		name: "Uyghur",
		ot: "UYG",
	},
	{
		html: "uz",
		name: "Uzbek",
		ot: "UZB",
	},
	{
		html: "ve",
		name: "Venda",
		ot: "VEN",
	},
	{
		html: "vi",
		name: "Vietnamese",
		ot: "VIT",
	},
	{
		html: "vo",
		name: "Volap\u00fck",
		ot: "VOL",
	},
	{
		html: "vro",
		name: "V\u00f5ro",
		ot: "VRO",
	},
	{
		html: "wbm",
		name: "Wa",
		ot: "WA",
	},
	{
		html: "wbr",
		name: "Wagdi",
		ot: "WAG",
	},
	{
		html: "crk",
		name: "West-Cree",
		ot: "WCR",
	},
	{
		html: "wdt",
		name: "Wendat",
		ot: "WDT",
	},
	{
		html: "cy",
		name: "Welsh",
		ot: "WEL",
	},
	{
		html: "wo",
		name: "Wolof",
		ot: "WLF",
	},
	{
		html: "wa",
		name: "Walloon",
		ot: "WLN",
	},
	{
		html: "wyn",
		name: "Wyandot",
		ot: "WYN",
	},
	{
		html: "khb",
		name: "L\u00fc",
		ot: "XBD",
	},
	{
		html: "xh",
		name: "Xhosa",
		ot: "XHS",
	},
	{
		html: "xpe",
		name: "Kpelle (Liberia)",
		ot: "XPE",
	},
	{
		html: "sah",
		name: "Sakha",
		ot: "YAK",
	},
	{
		html: "yo",
		name: "Yoruba",
		ot: "YBA",
	},
	{
		html: "crj",
		name: "Y-Cree",
		ot: "YCR",
	},
	{
		html: "ii",
		name: "Yi Modern",
		ot: "YIM",
	},
	{
		html: "zgh",
		name: "Standard Moroccan Tamazight",
		ot: "ZGH",
	},
	{
		html: "za",
		name: "Zhuang",
		ot: "ZHA",
	},
	{
		html: "zh-HK",
		name: "Chinese ; Hong Kong",
		ot: "ZHH",
	},
	{
		html: "zh-Hans",
		name: "Chinese ; Han (Simplified variant)",
		ot: "ZHS",
	},
	{
		html: "zh-Hant",
		name: "Chinese ; Han (Traditional variant)",
		ot: "ZHT",
	},
	{
		html: "zh-MO",
		name: "Chinese ; Macao",
		ot: "ZHTM",
	},
	{
		html: "zne",
		name: "Zande",
		ot: "ZND",
	},
	{
		html: "zu",
		name: "Zulu",
		ot: "ZUL",
	},
	{
		html: "zza",
		name: "Zazaki",
		ot: "ZZA",
	},
];
