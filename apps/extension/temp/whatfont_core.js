...
function _whatFont() {
  ...
  TestCanvas.prototype = {
    ...
    draw() {
      ...
      $.each(this.canvas_options, (opt, val) => {
        ctx[opt] = val;
      });
  ...
  TypeInfo.roundFloatWithPxUnit = (original) => {
    var number = Math.round(Number.parseFloat(original));

    if (isNaN(number)) {
      return "(unknown)";
    }
    return Math.round(Number.parseFloat(original)) + "px";
  };
  ...
  TypeInfo.prototype = {
    ...
    saveDesignToServer(cb) {
      $.getJSON(
        apiURL + "/designs/create?callback=?",
        {
          font: this.current,
          style: this.style,
          weight: this.weight,
  ...
  fs = {
        ...
        $.getJSON(
          ...
          (data) => {
            if (!data.errors) {
              ...
              $.each(data.kit.families, (i, family) => {
                $.each(family.css_names, (i, css) => {
    ...
    fontdeck() {
      ...
      $.each(projectIds, (i, projectId) => {
        $.getJSON(
          "http://fontdeck.com/api/v1/project-info?project=" +
            projectId +
            "&domain=" +
            domain +
            "&callback=?",
          (data) => {
            if (
              ...
              typeof data.provides !== "undefined"
            ) {
              $.each(data.provides, (i, font) => {
                var fontName = font.name,
                  slug = fontName.replace(/ /g, "-").toLowerCase(),
                  searchTerm = fontName.split(" ")[0],
                  fontURL =
                    font.url || "http://fontdeck.com/search?q=" + searchTerm;
  ...
  Tip.prototype = {
    ...
    update(newElement, event) {
      ...
      typeInfoCache[cacheId] = this.typeInfo =
        typeInfoCache[cacheId] || new TypeInfo(newElement);
  ...
  panel = {
    ...
    init() {
      ...
      panel.fontServiceIcons.Typekit = $("<span>")
        .addClass("service_icon service_icon_typekit")
        .text("Typekit");
      panel.fontServiceIcons.Google = $("<span>")
        .addClass("service_icon service_icon_google")
        .text("Google Web Fonts");
      panel.fontServiceIcons.Fontdeck = $("<span>")
    ...
    fontService(typeInfo, newPanel) {
      ...
      if (fontData) {
        $.each(fontData.services, (name, srv) => {
          $("<li>")
            .append(
              $("<a>")
                .append($(panel.fontServiceIcons[name]).clone())
                .attr("href", srv.url)
                .attr("target", "_blank")
            )
    ...
    fontFam(typeInfo, newPanel) {
      ...
      fHTML =
        "<div class=" +
        css.getClassName("fontfamily_list") +
        ">" +
        fHTML +
        "</div>";
    ...
    sizeLineHeight(typeInfo, newPanel) {
    ...
    color(typeInfo, newPanel) {
      ...
      r = Number.parseInt(match[1], 10).toString(16);
      g = Number.parseInt(match[2], 10).toString(16);
      b = Number.parseInt(match[3], 10).toString(16);
      r = r.length === 1 ? "0" + r : r;
      g = g.length === 1 ? "0" + g : g;
      b = b.length === 1 ? "0" + b : b;
      hex_color = "#" + r + g + b;
      ...
      sample
        ...
        .click(
          ((colors, color_type, value) => (e) => {
            color_type = (color_type + 1) % colors.length;
            value.text(colors[color_type]);

            e.preventDefault();
            return false;
          })(colors, color_type, value)
        )
        .click();
    },

    tweet(typeInfo, newPanel) {
      ...
      tweet_icon.click(() => {
        ...
        panel.takeScreenshot(newPanel.e, newPanel, () => {
          url +=
            "?text=" +
            encodeURIComponent(
              "I like this typography design with " + typeName + "."
            ) +
            "&url=" +
            encodeURIComponent(typeInfo.designURL || window.location.href) +
            "&via=What_Font";
    ...
    panelContent(typeInfo, newPanel) {
      $([
        "typePreview",
        "fontService",
        "fontFam",
    ...
    takeScreenshot(e, p, cb) {
      ...
      screenshot.takeScreenshotAroundPoint(
        (data) => {
          if (data) {
            p.screenshot = data;
            p.typeInfo.screenshot = data;
            p.typeInfo.saveDesignToServer(() => {
              cb(e, p);
            });
          } else {
            cb(e, p);
    ...
    init() {
      var exit = $.wfElem("div", "exit", "Exit WhatFont"),
        help = $.wfElem(
          "div",
          "help",
          "<strong>Hover</strong> to identify<br /><strong>Click</strong> to pin a detail panel"
        );
  ...
  ctrl = {
    ...
    init(options) {
      ...
      $.wfElem = (tag, className, content, attr) => {
  ...
  var screenshot = {
        ...
        try {
          ...
          h2c([document.body], {
            onrendered(canvas) {
              ...
              cb(
                canvas.toDataURL(
                  options.format || screenshot.format,
                  options.quality || screenshot.quality
                )
              );
            },
            proxy: undefined,
    ...
    takePartialScreenshot(cb) {
      panel.hideAll();

      window.setTimeout(() => {
        screenshot.capturer((dataURL) => {
          panel.showAll();
          cb(dataURL);
        });
      }, 0);
    ...
    takeCroppedScreenshot(cb, x1, y1, x2, y2, options) {
      var capturer, scrollTop, width, height;

      capturer = screenshot.captureAll
        ? screenshot.takeFullScreenshot
        : screenshot.takePartialScreenshot;
    ...
    takeScreenshotAroundPoint(cb, centerX, centerY, width, height, options) {
  ...
  _wf = {
    ...
    setHTML2Canvas(html2canvas) {
      h2c = html2canvas;
    },
    setScreenshot(options) {
