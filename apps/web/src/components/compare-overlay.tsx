 ...
 *
 * Fonts without an OS/2 table (or missing sxHeight/sCapHeight in it) report
 * these as undefined from fontkit — drop them rather than drawing a guide
 * line at a NaN position.
 */
function fontMetricLines(font: FontkitFont): FontMetricLine[] {
  ...
  ].filter((metric): metric is FontMetricLine => Number.isFinite(metric.value));
}
